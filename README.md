...
python export_model.py --run-id YOUR_RUN_ID
```
...
```bash
python generate_predictions.py --count 30
```
...
- `register_training_set.py` - Register training baseline
- `prediction_capture_example.py` - Example code for deployment script
- `export_model.py` - Export models from MLflow
- `setup_monitoring.py` - Interactive configuration
- `generate_predictions.py` - Generate test predictions
- `upload_ground_truth.py` - Register ground truth datasets
...
- Run `python setup_monitoring.py` to reconfigure
