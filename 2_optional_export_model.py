...
Usage:
    ...
    python 2_optional_export_model.py --run-id abc123def456
    python 2_optional_export_model.py --run-id abc123def456 --folder /mnt/artifacts
    python 2_optional_export_model.py --output /mnt/artifacts/model.pkl
"""
...
def export_model(run_id, output_path=None, folder=None):
    """
    Export a model from MLflow.

    Args:
        run_id: MLflow run ID
        output_path: Full path to save the model (overrides folder)
        folder: Directory to save model.pkl (default: current directory)
    ...
    if output_path:
        ...
        output_path = Path(folder) / "model.pkl"
    else:
...
def main():
    ...
    parser.add_argument("--output", help="Full output path (e.g., /mnt/artifacts/model.pkl)")
    parser.add_argument("--folder", help="Output folder (saves as model.pkl, default: current directory)")
    ...
    try:
        ...
        output_path = export_model(run_id, args.output, args.folder)
